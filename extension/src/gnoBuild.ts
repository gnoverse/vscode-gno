		...
		if (!buildWorkspace) {
			...
			if (editor.document.languageId !== 'go') {
				vscode.window.showInformationMessage(
					'File in the active editor is not a Gno file, cannot find current package to build'
				);
				return;
			}
		}
...
export async function goBuild(
	...
	const isTestFile = fileUri && fileUri.fsPath.endsWith('_test.gno');
	...
	if (!isMod) {
		...
		if (currentGoWorkspace && !isMod) {
			...
			outputChannel.error(
				`Not able to determine import path of current package by using cwd: ${cwd} and Gno workspace: ${currentGoWorkspace}`
			);
		}
	}
