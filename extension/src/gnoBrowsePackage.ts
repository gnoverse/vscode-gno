...
import { getAllPackages } from './gnoPackages';
...
function showPackageFiles(pkg: string, showAllPkgsIfPkgNotFound: boolean, workDir: string) {
	...
	if (!goRuntimePath) {
		return vscode.window.showErrorMessage(
			`Failed to run "gno list" to fetch packages as the "gno" binary cannot be found in either GNOROOT(${getCurrentGoRoot()}) or PATH(${getEnvPath()})`
		);
	}
