...
async function goListPkgs(workDir?: string): Promise<Map<string, PackageInfo>> {
	...
	try {
		...
		vscode.window.showErrorMessage(
			`Running gno list failed with "${err}"\nCheck if you can run \`gno ${args.join(
				' '
...
export function getImportPathToFolder(targets: string[], cwd?: string): Promise<Map<string, string>> {
	...
	if (!goRuntimePath) {
		console.warn(
			`Failed to run "gno list" to find packages as the "gno" binary cannot be found in either GOROOT(${getCurrentGoRoot()}) PATH(${getEnvPath()})`
		);
