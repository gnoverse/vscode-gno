...
import { applyCodeCoverageToAllEditors } from './gnoCover';
...
export async function goTest(testconfig: TestConfig): Promise<boolean> {
	...
	if (!goRuntimePath) {
		vscode.window.showErrorMessage(
			`Failed to run "gno test" as the "go" binary cannot be found in either GOROOT(${getCurrentGoRoot()}) or PATH(${getEnvPath()})`
		);
