...
async function golist(goCtx: GoExtensionContext): Promise<string[]> {
	...
	const COMMAND = 'gnopls.list_known_packages';
	if (languageClient && serverInfo?.Commands?.includes(COMMAND)) {
		try {
			...
			console.log(`error with gnopls.list_known_packages: ${e}`);
		}
	}
...
export const addImport: CommandFactory = (ctx, goCtx) => (arg: { importPath: string }) => {
	...
	p.then(async (imp) => {
		...
		const COMMAND = 'gnopls.add_import';
		if (languageClient && serverInfo?.Commands?.includes(COMMAND)) {
			try {
				const editor = vscode.window.activeTextEditor;
				if (!editor) {
				...
				console.log(`error executing gnopls.add_import: ${e}`);
			}
		}
	});
};
