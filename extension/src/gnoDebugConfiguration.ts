...
import { packagePathToGoModPathMap } from './gnoModules';
import { getToolAtVersion } from './gnoTools';
...
export class GoDebugConfigurationProvider implements vscode.DebugConfigurationProvider {
	static activate(ctx: vscode.ExtensionContext, goCtx: GoExtensionContext) {
		ctx.subscriptions.push(
			vscode.debug.registerDebugConfigurationProvider('gno', new GoDebugConfigurationProvider('gno'))
		);
		const registerCommand = createRegisterCommand(ctx, goCtx);
		registerCommand('gno.debug.pickProcess', () => pickProcess);
		registerCommand('gno.debug.pickGoProcess', () => pickGoProcess);
	}

	constructor(private defaultDebugAdapterType: string = 'gno') {}
	...
	public async pickConfiguration(): Promise<vscode.DebugConfiguration[]> {
		const debugConfigurations = [
			...
			{
				label: 'Gno: Attach to local process',
	...
	public async resolveDebugConfiguration(
		...
		if (!debugConfiguration || !debugConfiguration.request) {
			...
			if (!activeEditor || activeEditor.document.languageId !== 'gno') {
				return;
			}
		...
		const goConfig = getGnoConfig(folder && folder.uri);
		...
		const defaultConfig = vscode.extensions.getExtension(extensionId)?.packageJSON.contributes.configuration
			.properties['gno.delveConfig'].properties;
		...
		if (debugConfiguration['env'] && debugConfiguration['env']['GNOFLAGS']) {
			const resp = this.removeGcflags(debugConfiguration['env']['GNOFLAGS']);
			if (resp.removed) {
				debugConfiguration['env']['GNOFLAGS'] = resp.args;
				this.showWarning(
					'ignoreDebugGCFlagsWarning',
					"User specified build flag '-gcflags' in 'GNOFLAGS' is being ignored (see [debugging with build flags](https://github.com/golang/vscode-go/blob/master/docs/debugging.md#specifying-other-build-flags) documentation)"
				);
			}
		}
		...
		if (debugConfiguration['mode'] === 'auto') {
			...
			if (debugConfiguration['program'] && debugConfiguration['program'].endsWith('.gno')) {
			...
			debugConfiguration['mode'] = filename?.endsWith('_test.gno') ? 'test' : 'debug';
		}

		if (debugConfiguration['mode'] === 'test' && debugConfiguration['program'].endsWith('_test.gno')) {
