...
import { addGoStatus, initGoStatusBar, outputChannel, removeGoStatus } from './gnoStatus';
import { containsTool, getConfiguredTools, getImportPathWithVersion, getTool, Tool, ToolAtVersion } from './gnoTools';
import {
	getBinPath,
...
export async function installAllTools(updateExistingToolsOnly = false) {
	...
	let allTools = getConfiguredTools(getGnoConfig(), getGnoplsConfig());

	// exclude tools replaced by alternateTools.
	const alternateTools: { [key: string]: string } = getGnoConfig().get('alternateTools') ?? {};
...
export async function installTools(
	...
	for (const tool of missing) {
		...
		} else if (tool.name === 'gnopls' && !skipRestartGopls) {
			// Restart the language server if a new binary has been installed.
			vscode.commands.executeCommand('gno.languageserver.restart', RestartReason.INSTALLATION);
		}
	}
...
async function tmpDirForToolInstallation() {
	...
	const tmpGoModFile = path.join(toolsTmpDir, 'gno.mod');
...
export function updateGoVarsFromConfig(goCtx: GoExtensionContext): Promise<void> {
	const { binPath, why } = getBinPathWithExplanation('gno', false);
	const goRuntimePath = binPath;

	outputChannel.debug(`updateGoVarsFromConfig: found 'gno' in ${goRuntimePath}`);
	...
	return new Promise<void>((resolve, reject) => {
		cp.execFile(
			...
			['env', '-json', 'GNOPATH', 'GNOROOT', 'GNOPROXY', 'GNOBIN', 'GNOMODCACHE'],
			...
			(err, stdout, stderr) => {
				...
				if (envOutput.GOROOT && envOutput.GOROOT.trim()) {
					outputChannel.debug('setCurrentGNOROOT:', envOutput.GOROOT);
...
function getMissingTools(matcher?: (value: Tool) => boolean): Promise<Tool[]> {
	let keys = getConfiguredTools(getGnoConfig(), getGnoplsConfig());
	if (matcher) {
...
async function defaultInspectGoToolVersion(
	...
	const goCmd = getBinPath('gno');
	const execFile = util.promisify(cp.execFile);
	let debugInfo = 'gno version -m failed';
	try {
...
export async function shouldUpdateTool(tool: Tool, toolPath: string): Promise<boolean> {
	...
	const checkForUpdates = getCheckForToolsUpdatesConfig(getGnoConfig());
...
export async function suggestUpdates() {
	...
	const allTools = getConfiguredTools(getGnoConfig(), getGnoplsConfig());
	...
	// without prompting.
	const toolsManagementConfig = getGnoConfig()['toolsManagement'];
...
export async function listOutdatedTools(configuredGoVersion: GoVersion | undefined, allTools: Tool[]): Promise<Tool[]> {
	...
	const oldTools = await Promise.all(
		allTools.map(async (tool) => {
			...
			if (!m) {
				console.log(`failed to get gno tool version: ${toolPath}`);
				return;
			}
			const { goVersion } = m;
			if (!goVersion) {
			...
			const toolGoVersion = new GoVersion('', `gno version ${goVersion} os/arch`);
...
/*export async function maybeInstallVSCGO(
	...
	const progPath = path.join(extensionPath, 'bin', correctBinname('vscgno'));
	...
	env['GNOBIN'] = path.dirname(progPath);

	const importPath = allToolsInformation['vscgno'].importPath;
	const version =
		extensionMode !== vscode.ExtensionMode.Production
			? ''
			: extensionId !== 'gnolang.gno' || isPreview
			? '@master'
			: `@v${extensionVersion}`;
	...
	try {
		...
		console.log(`installing vscgno: ${args.join(' ')}`);
