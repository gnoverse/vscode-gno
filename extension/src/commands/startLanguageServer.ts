...
import { outputChannel, updateLanguageServerIconGoStatusBar } from '../gnoStatus';
import {
	buildLanguageClient,
	...
	return () => {
		if (!goCtx.languageServerIsRunning) {
			vscode.window.showErrorMessage(
				'"Gno: Start language server\'s maintainer interface" command is available only when the language server is running'
			);
			return;
		}
		vscode.commands.executeCommand('gopls.start_debugging', {}).then(undefined, (reason) => {
			vscode.window.showErrorMessage(
				`"Gno: Start language server's maintainer interface" command failed: ${reason}`
			);
		});
	};
};