...
export const playgroundCommand: CommandFactory = () => () => {
	...
	outputChannel.show();
	outputChannel.appendLine('Upload to the Gno Playground in progress...\n');

	const selection = editor.selection;
	const code = selection.isEmpty ? editor.document.getText() : editor.document.getText(selection);
	const config: vscode.WorkspaceConfiguration | undefined = getGnoConfig(editor.document.uri).get('playground');
