...
export class GoTestExplorer {
	static setup(context: ExtensionContext, goCtx: GoExtensionContext): GoTestExplorer {
		...
		const ctrl = vscode.tests.createTestController('gno', 'Gno');
		...
		context.subscriptions.push(
			vscode.commands.registerCommand('gno.test.showProfiles', async (item) => {
				if (!item) {
					await vscode.window.showErrorMessage('No test selected');
					return;
				}
		...
		context.subscriptions.push(
			vscode.commands.registerCommand('gno.test.captureProfile', async (item) => {
				if (!item) {
					await vscode.window.showErrorMessage('No test selected');
					return;
				}
		...
		context.subscriptions.push(
			vscode.commands.registerCommand('gno.test.deleteProfile', async (file) => {
				if (!file) {
		...
		context.subscriptions.push(
			vscode.commands.registerCommand('gno.test.showProfileFile', async (file: Uri) => {
		...
		const watcher = workspace.createFileSystemWatcher('**/*_test.gno', false, true, false);
	...
	protected async didChangeConfiguration(e: ConfigurationChangeEvent) {
		let update = false;
		this.ctrl.items.forEach((item) => {
			if (e.affectsConfiguration('gno.testExplorerPackages', item.uri)) {
				dispose(this.resolver, item);
				update = true;
			}
		});
	...
	private async documentUpdate(doc: TextDocument, ranges?: Range[]) {
		if (!doc.uri.path.endsWith('_test.gno')) {
			return;
		}
