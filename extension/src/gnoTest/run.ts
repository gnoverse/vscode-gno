...
export class GoTestRunner {
	...
	async debug(request: TestRunRequest, token?: CancellationToken) {
		if (!request.include) {
			await vscode.window.showErrorMessage('The Gno test explorer does not support debugging multiple tests');
			return;
		}
	...
	async run(request: TestRunRequest, token?: CancellationToken, options: ProfilingOptions = {}): Promise<boolean> {
		...
		const windowGoConfig = getGnoConfig();
		...
		for (const [pkg, items] of collected.entries()) {
			...
			const goConfig = getGnoConfig(pkg.uri);
			const flags = getTestFlags(goConfig);
			const includeBench = getGnoConfig(pkg.uri).get('testExplorer.alwaysRunBenchmarks');
			...
			if (subItems.length > 0 && Object.keys(tests).length + Object.keys(benchmarks).length > 1) {
				outputChannel.error(
					`The following tests in ${pkg.uri} failed to run, as gno test will only run a sub-test or sub-benchmark if it is by itself:`
				);
				Object.keys(tests)
				...
				vscode.window.showErrorMessage(
					`Cannot run the selected tests in package ${pkg.label} - see the Gno output panel for details`
				);
				continue;
			}
	...
	parseOutput(test: TestItem, output: string[]): TestMessage[] {
		...
		for (const line of output) {
			...
			const m = line.match(/^.*\s+(?<file>\S+\.gno):(?<line>\d+): (?<message>.*\n)$/);
			if (m?.groups) {
				const file =
