...
export class GoTestProfiler {
	...
	postRun() {
		...
		vscode.commands.executeCommand('setContext', 'gno.profiledTests', Array.from(this.runs.keys()));
		vscode.commands.executeCommand('setContext', 'gno.hasProfiles', this.runs.size > 0);
...
async function show(profile: string) {
	...
	const panel = vscode.window.createWebviewPanel('gno.profile', 'Profile', ViewColumn.Active);
...
class ProfileTreeDataProvider implements TreeDataProvider<TreeElement> {
	...
	getTreeItem(element: TreeElement): TreeItem {
		if (element instanceof File) {
			...
			item.contextValue = 'gno:test:file';
			item.command = {
				title: 'Open',
				command: 'gno.test.showProfileFile',
		...
		item.contextValue = 'gno:test:test';
