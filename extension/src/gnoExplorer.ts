...
export class GoExplorerProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
	...
	static setup(ctx: vscode.ExtensionContext) {
		...
		ctx.subscriptions.push(
			registerTreeDataProvider('gno.explorer', provider),
			registerCommand('gno.explorer.refresh', () => provider.update(true)),
			registerCommand('gno.explorer.open', (item) => provider.open(item)),
			registerCommand('gno.workspace.editEnv', (item) => provider.editEnv(item)),
			registerCommand('gno.workspace.resetEnv', (item) => provider.resetEnv(item))
		);
		executeCommand('setContext', 'gno.showExplorer', true);
		return provider;
	}
...
class EnvTree implements vscode.TreeItem {
	label = 'env';
	contextValue = 'gno:explorer:envtree';
...
function isEnvTree(item?: vscode.TreeItem): item is EnvTree {
	return item?.contextValue === 'gno:explorer:envtree';
}
...
class EnvTreeItem implements vscode.TreeItem {
	...
	constructor(public key: string, public value: string) {
		...
		this.contextValue = 'gno:explorer:envitem';
		if (GoEnv.fileVars.has(key)) {
			this.contextValue = 'gno:explorer:envfile';
...
function isEnvTreeItem(item?: vscode.TreeItem): item is EnvTreeItem {
	return item?.contextValue === 'gno:explorer:envitem';
}
...
class GoEnv {
	...
	static async get(uri?: vscode.Uri) {
		const toolsEnv = await getGnoConfig(uri)['toolsEnvVars'];
	...
	static fileVars = new Set(['GNOMOD', 'GNOWORK', 'GNOENV']);
	...
	private static vars = ['GNOPRIVATE', 'GNOMOD', 'GNOWORK', 'GNOENV', 'GNOTOOLCHAIN'];
}

class ToolTree implements vscode.TreeItem {
	label = 'tools';
	contextValue = 'gno:explorer:tools';
	collapsibleState = vscode.TreeItemCollapsibleState.Collapsed;
	iconPath = new vscode.ThemeIcon('tools');
}

function isToolTree(item?: vscode.TreeItem): item is ToolTree {
	return item?.contextValue === 'gno:explorer:tools';
}

class ToolTreeItem implements vscode.TreeItem {
	contextValue = 'gno:explorer:toolitem';
	...
	constructor({ name, version, goVersion, binPath, error }: ToolDetail) {
		...
		if (error) {
			const msg = `gno version -m failed: ${error}`;
...
function isToolTreeItem(item?: vscode.TreeItem): item is ToolTreeItem {
	return item?.contextValue === 'gno:explorer:toolitem';
}
