...
export async function runGoEnv(uri?: vscode.Uri, envvars: string[] = []): Promise<any> {
	const goExecutable = getBinPath('gno');
	if (!goExecutable) {
		console.warn(
			`Failed to run "gno env GNOMOD" to find mod file as the "gno" binary cannot be found in either GOROOT(${getCurrentGoRoot()}) or PATH(${getEnvPath()})`
		);
		return {};
	}
	...
	try {
		...
		vscode.window.showErrorMessage(`Failed to run "gno env ${args}": ${(e as Error).message}`);
		return {};
	}
}
...
export async function getModFolderPath(fileuri?: vscode.Uri, isDir?: boolean): Promise<string | undefined> {
	...
	const goModEnvJSON = await runGoEnv(pkgUri, ['GNOMOD']);
	let goModEnvResult =
		goModEnvJSON['GNOMOD'] === '/dev/null' || goModEnvJSON['GNOMOD'] === 'NUL' ? '' : goModEnvJSON['GOMOD'];
	if (goModEnvResult) {
