...
export const addTags: CommandFactory = () => (commandArgs: GoTagsConfig) => {
	...
	getTagsAndOptions(<GoTagsConfig>getGnoConfig()['addTags'], commandArgs).then(
...
export const removeTags: CommandFactory = () => (commandArgs: GoTagsConfig) => {
	...
	getTagsAndOptions(<GoTagsConfig>getGnoConfig()['removeTags'], commandArgs).then(([tags, options]) => {
		if (!tags && !options) {
...
function getCommonArgs(): string[] {
	...
	if (!editor.document.fileName.endsWith('.gno')) {
		vscode.window.showInformationMessage('Current file is not a Gno file.');
		return [];
	}
...
function runGomodifytags(args: string[]) {
	const gomodifytags = getBinPath('gnomodifytags');
	const editor = vscode.window.activeTextEditor;
	if (!editor) {
		return;
	}
	...
	const p = cp.execFile(gomodifytags, args, { env: toolExecutionEnvironment() }, (err, stdout, stderr) => {
		if (err && (<any>err).code === 'ENOENT') {
			promptForMissingTool('gnomodifytags');
			return;
		}
		if (err && (<any>err).code === 2 && args.indexOf('--template') > 0) {
			...
			promptForUpdatingTool('gnomodifytags');
			return;
		}
		if (err) {
