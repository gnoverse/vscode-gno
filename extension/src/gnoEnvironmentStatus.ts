...
import { installTool } from './gnoInstallTools';
...
export const chooseGoEnvironment: CommandFactory = () => async () => {
	...
	if (!ok) {
		vscode.window.showInformationMessage(`GNOROOT: ${getCurrentGoRoot()}. ${reason}`);
		return;
	}
...
export async function setSelectedGo(goOption: vscode.QuickPickItem, promptReload = true): Promise<boolean> {
	...
	if (goOption instanceof GoEnvironmentOption) {
		...
		const oldGoBin = fixDriveCasingInWindows(path.join(defaultUri?.fsPath ?? '', correctBinname('gno')));
	...
	if (promptReload) {
		const choice = await vscode.window.showWarningMessage(
			'Please reload the window to finish applying Gno version changes.',
			{
				modal: true
			},
			'Reload Window'
	...
	goEnvStatusbarItem.text = 'Gno: reload required';
	...
	return await vscode.window.withProgress(
		...
		async () => {
			...
			if (result) {
				...
				throw new Error(`Could not install ${goOption.binpath} - check logs in the "Gno" output channel`);
			}
			// run `goX.X download`
			...
			try {
				...
				throw new Error('Could not download Gno version.');
			}
			...
			let sdkPath = '';
			try {
				const { stdout } = await execFile(goXExecutable, ['env', 'GNOROOT'], {
			...
			const binpath = path.join(sdkPath, 'bin', correctBinname('gno'));
...
export async function updateIntegratedTerminal(terminal: vscode.Terminal): Promise<void> {
	if (
		...
		(isTerminalOptions(terminal.creationOptions) &&
			path.basename(terminal.creationOptions.shellPath || '') === correctBinname('gno'))
	) {
		return;
	}
	const gorootBin = path.join(getCurrentGoRoot(), 'bin');
	const defaultGoRuntime = getBinPathFromEnvVar('gno', defaultPathEnv, false);
...
export function formatGoVersion(version?: GoVersion): string {
	...
	if (versionWords.length > 1 && versionWords[0] === 'devel') {
		...
		return versionWords[1].startsWith('gno') ? `${versionWords[1].slice(2)}` : `${versionWords[1]}`;
	} else {
...
async function getSDKGoOptions(): Promise<GoEnvironmentOption[]> {
	...
	return subdirs.map(
		(dir: string) =>
			new GoEnvironmentOption(path.join(sdkPath, dir, 'bin', correctBinname('gno')), dir.replace('gno', 'Gno '))
	);
}
...
export async function getDefaultGoOption(): Promise<GoEnvironmentOption | undefined> {
	...
	return new GoEnvironmentOption(path.join(goroot, 'bin', correctBinname('gno')), formatGoVersion(version));
}
...
async function fetchDownloadableGoVersions(): Promise<GoEnvironmentOption[]> {
	...
	return webResults.reduce((opts, result: GoVersionWebResult) => {
		...
		const label = result.version.replace('gno', 'Gno ');
...
const STATUS_BAR_ITEM_NAME = 'Gno Update Notification';
...
export async function offerToInstallLatestGoVersion(ctx: Pick<vscode.ExtensionContext, 'subscriptions'>) {
	...
	if (options.length > 0) {
		...
		statusBarItem.text = 'New Gno version is available';
		statusBarItem.detail = versionsText;
		statusBarItem.command = {
			title: 'Upgrade',
			command: 'gno.promptforgoinstall',
			arguments: [options],
		...
		ctx.subscriptions.push(
			vscode.commands.registerCommand('gno.promptforgoinstall', () => {
				const download = {
					title: 'Download',
					async command() {
