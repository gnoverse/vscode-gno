...
func TestRun(t *testing.T) {
	if _, err := exec.LookPath("gnopls"); err != nil {
		t.Skipf("gnopls is not found (%v), skipping...", err)
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got, want := normalize(t, string(b)), normalize(t, `
			{
				"gnopls": {
					"type": "object",
					"markdownDescription": "Configure the default Go language server ('gnopls'). In most cases, configuring this section is unnecessary. See [the documentation](https://github.com/golang/tools/blob/master/gopls/doc/settings.md) for all available settings.",
					"scope": "resource",
					"properties": {
				       `+tc.out+`
					}
