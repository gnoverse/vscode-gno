...
export function runGoOutline(
  ...
  return new Promise((resolve, reject) => {
    ...
    p = cp.execFile(gooutline, gooutlineFlags, (err, stdout, stderr) => {
      try {
        if (err && (<any>err).code === "ENOENT") {
          promptForMissingTool(gooutline);
        }
...
export class GoLegacyDocumentSymbolProvider
  implements vscode.DocumentSymbolProvider
{
  ...
  public async provideDocumentSymbols(
    ...
    if (typeof this.includeImports !== "boolean") {
      const gnotoSymbolConfig = getGnoConfig(document.uri)["gotoSymbol"];
      this.includeImports = gnotoSymbolConfig
        ? gnotoSymbolConfig["includeImports"]
        : false;
    }
