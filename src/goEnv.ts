...
export function toolInstallationEnvironment(): NodeJS.Dict<string> {
  ...
  const toolsGopath = setupGoPath(env);
...
function newEnvironment(
  ...
  const env = addProcessEnv
    ? Object.assign({}, process.env, toolsEnvVars)
    : Object.assign({}, toolsEnvVars);
...
export async function setGOROOTEnvVar(configGOROOT: string) {
  ...
  if (!(await dirExists(goroot ?? ""))) {
    vscode.window.showWarningMessage(
      `go.goroot setting is ignored. ${goroot} is not a valid GOROOT directory.`,
    );
    return;
  }
  const neverAgain = { title: "Don't Show Again" };
  const ignoreGOROOTSettingWarningKey = "ignoreGOROOTSettingWarning";
  const ignoreGOROOTSettingWarning = getFromGlobalState(
    ignoreGOROOTSettingWarningKey,
  );
...
function setupGoPath(env: NodeJS.Dict<string>): string | null {
  ...
  return toolsGopath; // Return the toolsGopath (or null if not set)
}
